...
char font8x8_basic_mirror[128][8] = {
	{ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   /* U+0000 (nul)*/
	{ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   /* U+0001*/
	{ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   /* U+0002*/
	{ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   /* U+0003*/
...
void OLEDRefresh(oled_t *oled)
{
	write(oled->fb_fd,oled->buf, 12*16);
	lseek(oled->fb_fd, 0, SEEK_SET);
	return;
}
void OLEDchar(oled_t *oled, int x, int y, char c)
{
	if ((x > 11) | ( y > 1) )
	{
		return;
	...
	for (i = 0; i < 8; i ++ )
	{
		oled->buf[i+(1-y)*8][11-x] = font8x8_basic_mirror[c][i];
	}
	return;
}
void OLEDstring(oled_t *oled, int x, int y, char *str)
{
	int i;
	for (i = 0; i < strlen(str); i ++ )
	{
		OLEDchar(oled, x + i, y, str[i]);
	}
	return;
}
...
void OLEDline(oled_t *oled, int line)
{
	int i,j;
	for (i=0; i<8; i ++ )
	{
		for (j=0; j< 12; j ++ )
			oled->buf[i+(1-line)*8][j] = 0;
	}
